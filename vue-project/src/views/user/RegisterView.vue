<template>
    ...
    <div 
        ...
        <div class=" transition-all flex flex-col gap-x-1 py-5 md:rounded-xl select-none" style="width: 100%;max-width: 500px;height: 100%;max-height: 600px;">
            ...
            <!-- 表单 -->
            <p class=" text-center text-3xl py-14 font-bold text-white">注册</p>
            <div class="transition-all flex flex-col px-10 md:px-20 gap-y-5">
                ...
                <a-input v-model:value="userInput.userName" placeholder="用户名" class="rounded-md" size="large" />

                <a-input-password v-model:value="userInput.userPassword" placeholder="密码" class="rounded-md" size="large" />
                ...
                <a-button type="primary" html-type="submit" style="width: 100%;" @click="startRegister()" class=" bg-blue rounded-md" 
                    :disabled="userInput.userEmail==''||userInput.userName==''||userInput.userPassword==''||userInput.verifyCode==''">注册</a-button>
            </div>
...
import {getMailVerify,register} from '../../request/api/user'
...
async function startRegister(){
    let res = await register(userInput.value);
    if(res.data.success){
        openNotification('success','注册成功');
        router.go(-1);
    }else{
